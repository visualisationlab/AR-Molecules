...
public class MoleculeBehaviour : MonoBehaviour {
	...
	public MoleculeBehaviour moleculeCopyBehaviour;
	...
	void addAtom(string atomString){
		...
		atomInstance.transform.localScale = new Vector3 (scale, scale, scale);
		...
		//ensure the atom is positioned relative to the parent.
...
//			moleculeCopy.GetComponent<MoleculeBehaviour> ().stopAnimation ();
			...
			moleculeCopyBehaviour = moleculeCopy.GetComponent<MoleculeBehaviour> ();
			...
			moleculeCopyBehaviour.destroyAnimationPoints();
...
//		}
	...
	// Removes the rotation axes from all atoms.
	public void destroyAnimationPoints(){

		// disable the molecule copy if there is one.
		setCopyEnabled(false);

		//destory all axis gameObjects
		foreach (GameObject symmetryPoint in symmetryPoints) {
			Destroy (symmetryPoint);
		}
